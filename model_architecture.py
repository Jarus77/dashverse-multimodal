...
Components:
1. EngraingDataset - PyTorch dataset from JSONL/metadata
...
class SimpleTokenizer:
    """Simple tokenizer for captions (can be replaced with huggingface)"""
...
class ImageDecoder(nn.Module):
    """Decode latent space to images (simplified diffusion-like decoder)"""
    ...
    def __init__(self, latent_dim=1024, image_channels=3):
        ...
        # Expand latent to spatial features
        self.fc = nn.Linear(latent_dim, 512 * 32 * 32)
    ...
    def forward(self, z):
        ...
        """
        features = self.fc(z)  # (B, 512*32*32)
        features = features.view(-1, 512, 32, 32)  # (B, 512, 32, 32)
        image = self.decoder(features)  # (B, 3, 512, 512)
        return image


class TextDecoder(nn.Module):
    """Decode latent space to caption tokens"""
    ...
    def __init__(self, latent_dim=1024, vocab_size=8000, max_length=100, embedding_dim=512):
        ...
        self.fc_latent = nn.Linear(latent_dim, embedding_dim)
    ...
    def forward(self, z, caption_tokens=None):
        ...
        hidden = self.fc_latent(z)  # (B, embedding_dim)
        hidden = hidden.unsqueeze(1)  # (B, 1, embedding_dim)
        
        # If training with teacher forcing
        if caption_tokens is not None:
            embeddings = self.token_embedding(caption_tokens)  # (B, max_length, embedding_dim)
            embeddings = torch.cat([hidden, embeddings[:, :-1, :]], dim=1)  # (B, max_length, embedding_dim)
        else:
            ...
            for _ in range(self.max_length - 1):
                ...
                next_token_logits = self.output_projection(out[:, -1:, :])  # (B, 1, vocab_size)
                next_token_idx = next_token_logits.argmax(dim=-1)  # (B, 1)
                next_embedding = self.token_embedding(next_token_idx)  # (B, 1, embedding_dim)
                embeddings = torch.cat([embeddings, next_embedding], dim=1)  # (B, t+1, embedding_dim)
        
        logits = self.output_projection(embeddings)  # (B, max_length, vocab_size)
        return logits


class MultimodalModel(nn.Module):
    """Unified multimodal model: image + caption from shared latent"""
    ...
    def forward(self, images, caption_tokens=None):
        ...
        Returns:
            Dict with 'image_recon' and 'caption_logits'
        """
        # Encode to latent
...
def main():
    ...
    logger.info("="*70)
    logger.info("TESTING DATA LOADER & MODEL")
    logger.info("="*70 + "\n")
